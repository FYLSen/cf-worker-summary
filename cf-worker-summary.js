...
class RequestHandler {
    ...
    async generateNewSummary(articleUrl, langCode) {
        ...
        const language = this.env.LANGUAGE_DETECTOR_URL ? await LanguageHelper.fetch(this.env.LANGUAGE_DETECTOR_URL, langCode) : langCode;
...
class LanguageHelper {
    ...
    static async fetch(url, langCode) {
        try {
            const language = await this.fetchFromLanguageDetector(url, langCode);
            return language;
        } catch (error) {
    ...
    static async fetchFromLanguageDetector(url, langCode) {
        try {
            const response = await fetch(`${url}?type=text&languageCode=${langCode}`);
            return response.ok ? await response.text() : langCode;
        } catch (error) {
            console.error('Language detector service error:', error);
            return null;
        }
    }
}
...
class CacheManager {
    ...
    `;
        await this.db.prepare(query)
            .bind(articleUrl, summary, model, langCode, Date.now())
            .run();
    }
}
